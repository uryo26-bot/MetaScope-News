...
import { cropCategoryStyles } from "../../data/crops";

export type CropCardItem = {
  id: string;
  name: string;
  nameEn: string;
  icon: string;
...
export function CropCard({ item }: { item: CropCardItem }) {
  const style = cropCategoryStyles[item.category];
  const classes = [
  ...
  return (
    <Link href={`/agricchart/crop/${item.id}`} className={classes}>
